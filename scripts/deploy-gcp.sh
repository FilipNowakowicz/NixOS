...
echo "  4. Remove bootstrap metadata:"
echo "     $(cd infra && tofu output -raw ssh_host_key_removal_cmd 2>/dev/null || echo 'cd infra && tofu output ssh_host_key_removal_cmd')"
echo "  5. Fill in real secrets: sops hosts/homeserver-gcp/secrets/secrets.yaml"
echo "  6. Later updates: deploy '.#homeserver-gcp' && bash scripts/check-host-drift.sh homeserver-gcp"
