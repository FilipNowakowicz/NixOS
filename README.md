...
1.  **Create a new theme file** in `home/theme/themes/`, following the structure of the existing themes (e.g., `nighthawks.nix`). A theme requires a `name`, `colors` set, an `ansiColors` palette, a `colorscheme`, and a path to a `wallpaper`.
2.  **Rebuild** so Home Manager generates the theme's assets. The theme is then auto-discovered and switchable via `theme-switch` (a runtime switch needs the assets built first).
