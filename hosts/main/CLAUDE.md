...
runtime. It disables Tailscale, SSH, Bluetooth, observability, and backups;
enables AppArmor and kernel hardening; auto-connects Mullvad with lockdown on;
and starts a Tor SOCKS5 proxy on `127.0.0.1:9050` with `proxychains` wired to
it (`strict_chain`, `proxy_dns`).
...
# Use a security tool through Tor
proxychains nmap -sT -Pn <target>

# Start Whonix (Gateway first, then Workstation)
